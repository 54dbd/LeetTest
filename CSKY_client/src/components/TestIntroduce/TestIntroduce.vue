      ...
      <el-row :gutter="20">
        <el-col class="elCol1" :span="12" :push="2">
          <el-card shadow="always">
            <div>
              <MarkDown :text="question" class="content"  v-if="qtype!='图片题'"/>
          ...
          <el-row class="changePage">
            <router-link @click.native="flush" :to="{path: '/test',query: {tid: parseInt(this.tid)-1}}">
          ...
          <el-row class="analysis">
            <el-card v-if="correct==true">
              ...
              <div v-if="analysis">
                <MarkDown :text="analysis" class="content"  v-if="qtype!='图片题'"/>
        ...
        <el-col class="elCol2" :span="10" :push="2">
          <el-card shadow="always">
            <el-descriptions title="题目信息" direction="vertical" :column="1" size="medium">
              <el-descriptions-item label="正确率"><span style="color: #77b72c">{{accuracy+'%'}}</span></el-descriptions-item>
              <el-descriptions-item label="正确人数"><span style="color: #77b72c">{{correctnum}}</span>
              </el-descriptions-item>
              <el-descriptions-item label="做题人数"><a rel="nofollow" target="_blank"
                                                    style="color: #4cb9fc; text-decoration: none;">{{num}}</a>
              </el-descriptions-item>
              <el-descriptions-item v-if="points!=null" label="知识点"><a rel="nofollow"  target="_blank"
                                                    style="color: #4cb9fc; text-decoration: none;">{{points}}</a>
              </el-descriptions-item>
            </el-descriptions>
          </el-card>
          <el-button v-if="atype==1" type="success" class="submit" @click="submitChoice">提交</el-button>
          <el-button v-if="atype==0" type="success" class="submit" @click="submitText">提交</el-button>
...
export default {
  ...
  methods:{
    ...
    async submitText(){
      ...
      //获得答案与回答的匹配程度,30%权重
      var weight_1 = this.strSimilarity2Percent(this.textarea,this.answer)*0.3
    ...
    async submitChoice(){
      ...
      if (choice.length!==this.answer.length){
        ...
        if(j===choice.length){//发现answer中不存在
          this.correct=false;
    ...
    //关键词模糊匹配
    fuzzyMatch(str,key){
      let index=-1, flag=false, count=0;
